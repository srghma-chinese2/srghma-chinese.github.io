...
function r12345_showChar(s) {
  const s2 = encodeURIComponent(s).replace(/%/g,'')
  const x = 'http://www.r12345.com/datafile/zy/'+s2.substr(5)+'/'+s2+'.png'
  return `<img src="${x}">`
}
...
function showText(hanzi, containerElement, text) {
  ...
  if (r12345_chars.includes(hanzi)) {
    text = text + r12345_showChar(hanzi)
  }
